...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 min-w-0 flex flex-col min-h-screen">
        ...
        <div class="flex flex-col gap-3 w-full">
                ...
                <div class="bg-white rounded-xl border border-[#E5E9EB] p-4 flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex flex-col gap-0.5 min-w-[180px]">
                        <div class="flex items-center gap-2">
                            <span class="text-[14px] font-bold text-[#0F172A]">₹{{ number_format($deposit->amount, 2) }}</span>
                            <span class="text-[10.5px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border {{ $pillClasses }}">{{ $deposit->status }}</span>
                        </div>
                        <span class="text-[12px] text-[#64748B]">{{ $deposit->phone }} &middot; {{ $deposit->method_label }}</span>
                        <span class="text-[11.5px] font-mono text-[#334155]">UTR: {{ $deposit->utr }}</span>
                        <span class="text-[11px] text-[#94A3B8]">{{ $deposit->submitted_at->format('d M Y, h:i A') }}</span>
                    </div>
            ...
            @empty
                <p class="text-[13.5px] text-[#94A3B8] italic">No {{ $status }} deposit requests.</p>
            @endforelse
        </div>
