    ...
    <main class="flex-1 min-w-0 flex flex-col min-h-screen">
        ...
        <form method="POST" action="{{ $plan->exists ? route('admin.plans.update', $plan) : route('admin.plans.store') }}" enctype="multipart/form-data" class="flex flex-col gap-3.5 bg-white rounded-2xl border border-[#E5E9EB] p-6 min-w-0">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                    <label for="title" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Title<x-info-tip text="The name of this plan. Customers will see this name everywhere - on the Home page, Explore page, and Plan Details page." /></label>
                ...
                <div>
                    ...
                    @endphp
                    <label for="badge-select" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Category<x-info-tip text="Pick a group for this plan, like 'Savings' or 'Gold'. Example: it lets customers filter plans by type on the Explore page." /></label>
            ...
            <div>
                <label for="subtitle" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Subtitle<x-info-tip text="A short line of text shown under the plan name. Example: for 'Dream Superbike' plan, the subtitle could be 'Own your dream ride faster'." /></label>
            ...
            <div class="mt-2 pt-3 border-t border-[#E5E9EB]">
                ...
                <div id="fixed-investment-section" class="rounded-xl border border-[#0A5C66]/20 bg-[#0A5C66]/[0.03] p-3.5 mb-3.5">
                    ...
                    <div class="max-w-xs">
                        <label for="investment_amount" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Investment (₹, one-time)<x-info-tip text="Only for Fixed plans. Every customer who buys this plan pays this exact amount. Example: type 199 and every customer pays ₹199 - no more, no less." /></label>
            ...
            <div id="flexible-investment-section" class="rounded-xl border border-[#0A5C66]/20 bg-[#0A5C66]/[0.03] p-3.5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                    <label for="min_investment_amount" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Min investment (₹)<x-info-tip text="Only for Flexible plans. The smallest amount a customer is allowed to invest. Example: type 100 and no customer can invest less than ₹100." /></label>
                ...
                <div>
                    <label for="max_investment_amount" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Max investment (₹)<x-info-tip text="Only for Flexible plans. The biggest amount a customer is allowed to invest. Example: type 1000 and no customer can invest more than ₹1,000. You must fill this along with Min investment." /></label>
                ...
                <div class="sm:col-span-2">
                    <label for="slider_step" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Slider step (₹, optional)<x-info-tip text="How much the amount jumps when the customer moves the slider. Example: type 100 and the customer can only pick ₹100, ₹200, ₹300 and so on - not ₹150. Leave empty and the system picks a good jump size for you." /></label>
                ...
                <label class="sm:col-span-2 flex items-center gap-2.5 h-11 px-3.5 rounded-lg border border-[#CBD5E1] bg-white has-[:checked]:border-brand has-[:checked]:bg-brand/5 cursor-pointer transition-colors w-fit">
                    ...
                    <x-info-tip text="Turn this ON so customers can keep adding money into the SAME investment again and again, like a monthly savings habit. All the money they add stays together as ONE investment with ONE finish date. Turn this OFF and every new payment becomes a brand new, separate investment. Only works on Flexible plans." />
                </label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                    <label for="lock_duration" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Lock duration<x-info-tip text="Just text that customers see, like '1 Day' or '12 Months'. Important: typing here does NOT change how long the plan actually runs - it's only a label. The real length is set by Term (days) or Duration options." /></label>
                ...
                <div>
                    <label for="sort_order" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Sort order<x-info-tip text="Controls which plan shows first. Example: a plan with sort order 1 shows before a plan with sort order 5." /></label>
            ...
            <div class="max-w-xs">
                <label for="status" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Availability<x-info-tip text="Controls who can see and buy this plan. Draft = only you (admin) can see it. Active = everyone can see and buy it. Hidden = customers can only buy it if you send them a direct link, it won't show in lists. Expired = nobody can see or buy it." /></label>
            ...
            <div class="pt-4 mt-1 border-t border-[#E5E9EB]">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                    <div>
                        <label for="plan_type" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Plan type<x-info-tip text="Choose what kind of plan this is. Regular = a normal plan, no special rules. Trust Builder = a special 1-day plan - usually customers must buy another plan first. Growth = a normal plan, often used to unlock Trust Builder." /></label>
                    ...
                    <div>
                        <label for="max_purchase_per_user" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Max purchases per user<x-info-tip text="How many times ONE person can buy this plan. Example: type 1 so each customer can only buy it once, ever. Leave empty for no limit." /></label>
                    ...
                    <div>
                        <label for="cooldown_days" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Cooldown (days)<x-info-tip text="After buying this plan, how many days a customer must wait before they can buy it again. Example: type 7 and a customer must wait 7 days before buying it a second time. Leave empty if there's no waiting time." /></label>
                ...
                <label class="flex items-center gap-2.5 h-11 px-3.5 rounded-lg border border-[#CBD5E1] has-[:checked]:border-brand has-[:checked]:bg-brand/5 cursor-pointer transition-colors w-fit mt-3">
                    ...
                    <x-info-tip text="Turn this ON if customers must buy a different plan FIRST, before they're allowed to buy this one." />
                </label>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mt-3">
                    <div>
                        <label for="requires_plan_id" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Requires purchase of<x-info-tip text="Pick which plan the customer must already own. This only works when the checkbox above is turned ON." /></label>
                    ...
                    <div>
                        <label for="unlock_message" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Unlock popup message<x-info-tip text="The message a customer sees in a popup when they try to buy this plan without unlocking it first." /></label>
            ...
            <div class="pt-4 mt-1 border-t border-[#E5E9EB]">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                    <div>
                        <label for="marketing_badge" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Marketing badge<x-info-tip text="A small tag shown on the plan card, like a sticker. Example: 'Most Popular' or 'Trending'. Leave empty if you don't want a tag." /></label>
                    ...
                    <div>
                        <label for="risk_level" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Risk level<x-info-tip text="Shows customers if this plan is Low, Medium, or High risk. This is just for showing information - it does not change any numbers." /></label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mt-3.5">
                    <div>
                        <label for="start_date" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Start date<x-info-tip text="The plan will only appear and be buyable ON or AFTER this date. Leave empty if there's no start date." /></label>
                    ...
                    <div>
                        <label for="end_date" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">End date<x-info-tip text="The plan will stop appearing and stop being buyable AFTER this date. Leave empty if there's no end date." /></label>
                ...
                <div class="flex flex-wrap gap-3 mt-3">
                    <label class="flex items-center gap-2.5 h-11 px-3.5 rounded-lg border border-[#CBD5E1] has-[:checked]:border-brand has-[:checked]:bg-brand/5 cursor-pointer transition-colors w-fit">
                        ...
                        <x-info-tip text="Turn this ON so the system automatically pays the customer's wallet and closes the plan when it finishes - you don't have to do anything. Turn this OFF and it stays open until you handle it yourself later." />
                    </label>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mt-3.5 border-t border-[#E5E9EB] pt-3.5">
                    <div>
                        <label for="max_purchases" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">
                            Max total purchases (catalog-wide, optional)<x-info-tip text="The total number of times this plan can be bought by ALL customers put together. Example: type 100 and only 100 people total can ever buy it (not 100 each - 100 in total). This is different from 'Max purchases per user' above, which is per-person. Leave empty for no limit." />
                        </label>
            ...
            {{-- ================= Multiple durations (max 4) =================
                 ...
                 or Investment mode = Fixed. Trust Builder always forces exactly
                 1 Day server-side; Fixed plans always use a single rate/term from
                 the top-level Growth rate + Term days fields instead of a
                 per-term choice (both collapsed to one row by
                 PlanManagementController::syncDurations()) - so this whole section
                 is simply irrelevant for either, not just "locked". Only Flexible,
                 non-Trust-Builder plans can define real multi-term rows here -
                 required for Flexible, since the purchase flow computes each
                 buyer's return from a Duration row's rate. --}}
            <div class="pt-4 mt-1 border-t border-[#E5E9EB]" id="duration-options-block">
                <h2 class="font-poppins font-bold text-[14px] text-[#0F172A] mb-1">Duration options (max 4)<x-info-tip text="Let customers pick their own term - like '3 Months' or '6 Months' or '12 Months'. Each choice has its own days and its own profit rate." /></h2>

                <div class="mb-3 flex items-center gap-2 px-3.5 py-2.5 rounded-lg bg-amber-50 border border-amber-200">
                    <i class="bi bi-exclamation-triangle-fill text-amber-600 text-[13px]"></i>
                    <span class="text-[12.5px] font-semibold text-amber-700">Required: at least one row below, or the plan won't save. Flexible purchases compute their return from a Duration row's rate - without one there's nothing to compute against.</span>
                </div>
                    ...
                    @for ($i = 0; $i < 4; $i++)
                        ...
                        <div data-duration-row class="grid grid-cols-2 sm:grid-cols-6 gap-2.5 items-end p-3 rounded-lg border border-[#E5E9EB]">
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-[10.5px] font-semibold text-[#64748B] mb-1">Default<x-info-tip text="Which of these choices is already picked for the customer when they open the page." /></label>
                                <input type="radio" name="duration_default" value="{{ $i }}" class="accent-brand w-4 h-4"
                                    {{ (string) $i === (string) $defaultDurationIndex ? 'checked' : '' }}>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-[10.5px] font-semibold text-[#64748B] mb-1">Label<x-info-tip text="The text customers see on the button. Example: '3 Months'." /></label>
                            ...
                            <div>
                                <label class="block text-[10.5px] font-semibold text-[#64748B] mb-1">Days<x-info-tip text="How many days THIS ONE choice lasts." /></label>
                                <input type="number" name="durations[{{ $i }}][duration_days]" min="1" value="{{ old("durations.$i.duration_days", $d?->duration_days) }}"
                                    class="w-full h-9 rounded-lg border border-[#CBD5E1] px-2.5 text-[13px] text-[#0F172A] outline-none focus:border-brand focus:ring-2 focus:ring-brand/15">
                            </div>
                            <div>
                                <label class="block text-[10.5px] font-semibold text-[#64748B] mb-1">Rate %<x-info-tip text="The profit rate for THIS ONE choice only. It can be different from the main Growth rate above." /></label>
            ...
            <div class="pt-4 mt-1 border-t border-[#E5E9EB]">
                ...
                <label class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Highlight chips (up to 6)<x-info-tip text="Small tags shown on the plan page. Example: '24x7 Support' or 'Instant Withdrawal'. You can leave some empty." /></label>
                ...
                <div class="mb-3.5">
                    <label for="terms" class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">Terms<x-info-tip text="The rules and conditions text shown to customers on the plan page." /></label>
                ...
                <label class="block text-[12.5px] font-semibold text-[#334155] mb-1.5">FAQs (up to 4)<x-info-tip text="Common questions and answers shown to customers on the plan page. You can leave some empty." /></label>
...
(function () {
    ...
    // --- Duration Type (Trust Builder locks to 1 Day, Fixed uses a single
    //     top-level rate/term - hide the whole Duration options section
    //     rather than show a "locked" message, since it doesn't apply at
    //     all for either. Only Flexible plans define real per-term rows.) ---
    ...
    var PLAN_TYPE_HINTS = {
        '': '',
        'trust_builder': 'This plan always lasts exactly 1 day and pays automatically. The Duration options below are hidden because they don\'t apply here.',
        'growth': ''
    };
