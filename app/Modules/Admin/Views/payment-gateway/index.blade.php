    ...
    <main class="flex-1 min-w-0 flex flex-col min-h-screen">
        ...
        <form method="POST" action="{{ route('admin.payment-gateway.settings') }}" class="flex flex-col gap-4 bg-white rounded-2xl border border-[#E5E9EB] p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div class="rounded-xl border border-[#E5E9EB] p-4 flex flex-col gap-3">
                    <div class="flex items-center gap-2">
                        <i class="fa-brands fa-bitcoin text-[#0A5C66] text-[13px]"></i>
                        <span class="text-[13.5px] font-bold text-[#0F172A]">USDT range</span>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label for="usdt_min_amount" class="block text-[11.5px] font-semibold text-[#334155] mb-1">Min ₹</label>
                            <input type="number" name="usdt_min_amount" id="usdt_min_amount" min="0" step="0.01" placeholder="0" value="{{ old('usdt_min_amount', $settings['usdt_min_amount']) }}"
                                class="w-full h-10 rounded-lg border border-[#CBD5E1] px-3 text-[14px] text-[#0F172A] outline-none transition-colors focus:border-brand focus:ring-2 focus:ring-brand/15">
                            @error('usdt_min_amount')<p class="text-[12px] font-semibold text-red-500 mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="usdt_max_amount" class="block text-[11.5px] font-semibold text-[#334155] mb-1">Max ₹</label>
                            <input type="number" name="usdt_max_amount" id="usdt_max_amount" min="0" step="0.01" placeholder="No limit" value="{{ old('usdt_max_amount', $settings['usdt_max_amount']) }}"
                                class="w-full h-10 rounded-lg border border-[#CBD5E1] px-3 text-[14px] text-[#0F172A] outline-none transition-colors focus:border-brand focus:ring-2 focus:ring-brand/15">
                            @error('usdt_max_amount')<p class="text-[12px] font-semibold text-red-500 mt-1">{{ $message }}</p>@enderror
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col gap-3">
            <div class="flex items-center justify-between gap-3">
                <h2 class="font-poppins font-bold text-[15px] text-[#0F172A]">USDT accounts</h2>
                <a href="{{ route('admin.payment-gateway.usdt-accounts.create') }}" class="h-9 px-3.5 rounded-lg bg-brand text-white font-semibold text-[12.5px] hover:bg-brand-light transition-colors active:scale-[0.99] flex items-center gap-2">
                    <i class="fa-solid fa-plus text-[11px]"></i> Add USDT account
                </a>
            </div>

            <div class="flex flex-col gap-3">
                @forelse ($usdtAccounts as $account)
                    <div class="bg-white rounded-xl border border-[#E5E9EB] p-4 flex items-center gap-4 flex-wrap {{ $account->is_active ? '' : 'opacity-60' }}">
                        @if ($account->qrImageUrl())
                            <img src="{{ $account->qrImageUrl() }}" alt="{{ $account->usdt_address }}" class="w-14 h-14 rounded-lg object-cover shrink-0 border border-[#E5E9EB]">
                        @else
                            <div class="w-14 h-14 rounded-lg bg-[#0A5C66]/10 flex items-center justify-center shrink-0">
                                <i class="fa-brands fa-bitcoin text-[20px] text-[#0A5C66]"></i>
                            </div>
                        @endif
                        <div class="flex flex-col gap-0.5 min-w-[200px] flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-[14px] font-bold text-[#0F172A] font-mono">{{ $account->usdt_address }}</span>
                                <span class="text-[10.5px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border {{ $account->is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200' }}">
                                    {{ $account->is_active ? 'Active' : 'Disabled' }}
                                </span>
                            </div>
                            <span class="text-[12px] text-[#64748B]">{{ $account->display_name ?: 'No display name' }}{{ $account->qr_image ? '' : ' · No QR uploaded' }}</span>
                        </div>

                        <div class="flex gap-2 shrink-0">
                            <form method="POST" action="{{ route('admin.payment-gateway.usdt-accounts.move', $account) }}">
                                @csrf
                                <input type="hidden" name="direction" value="up">
                                <button type="submit" {{ $loop->first ? 'disabled' : '' }} class="h-9 w-9 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 transition-colors active:scale-95 flex items-center justify-center disabled:opacity-30 disabled:pointer-events-none" title="Move up (higher priority)">
                                    <i class="fa-solid fa-arrow-up text-[12px]"></i>
                            ...
                            <a href="{{ route('admin.payment-gateway.usdt-accounts.edit', $account) }}" class="h-9 px-3.5 rounded-lg border border-slate-200 text-slate-600 text-[12.5px] font-bold hover:bg-slate-50 transition-colors active:scale-95 flex items-center">Edit</a>
                            <form method="POST" action="{{ route('admin.payment-gateway.usdt-accounts.toggle-active', $account) }}">
                                @csrf
                                <button type="submit" class="h-9 px-3.5 rounded-lg border text-[12.5px] font-bold transition-colors active:scale-95 {{ $account->is_active ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50' }}">
                                    {{ $account->is_active ? 'Disable' : 'Enable' }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('admin.payment-gateway.usdt-accounts.delete', $account) }}" onsubmit="return confirm('Delete this USDT account?');">
                                @csrf
                                <button type="submit" class="h-9 px-3.5 rounded-lg border border-red-200 text-red-600 text-[12.5px] font-bold hover:bg-red-50 transition-colors active:scale-95">Delete</button>
                            </form>
                        </div>
                ...
                @empty
                    <p class="text-[13.5px] text-[#94A3B8] italic">No USDT accounts yet.</p>
                @endforelse
            </div>
        </div>
