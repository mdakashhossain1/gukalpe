...
- **Schema** (`2026_08_14_000900_add_workflow_fields_to_referral_commissions_table.php`): added `uuid` (route key, same pattern as DepositRequest), `source` (plan_purchase/deposit), `status` (pending/paid/rejected/reversed), `reason`, `reviewed_at`, `deposit_request_id`. `user_plan_id` became nullable (deposit-sourced rows have none); `deposit_request_id` got its own unique index as that source's double-credit guard. Existing rows backfilled to `status=paid, source=plan_purchase` since they were already instantly credited under the old model.
  - **SQLite gotcha worth remembering**: adding a `unique()` + `constrained()` FK column to an existing SQLite table in one `Schema::table` ADD-COLUMN pass silently drops the constraint (SQLite's ALTER TABLE ADD COLUMN can't carry UNIQUE/enforced-FK). Fix: add the column plain first, then apply `.unique()->change()` in a separate pass - same two-phase shape the pre-existing `uuid` migrations already used, just easy to violate when adding a *new* column instead of retrofitting one. Went through several rollback/reset cycles discovering this; final migration verified clean in both directions (`migrate` / `migrate:rollback` round-tripped correctly) before moving on.
- **Money flow**: `PlanPurchaseController::creditReferralCommissionIfEligible()` now creates a `STATUS_PENDING` row and no longer calls `WalletBalance::credit()` directly. New `AdminController::creditReferralCommissionForDeposit()`, called from `approveDeposit()`, mirrors it for the deposit source (own enable toggle, same min/max qualifying-amount gate, "first qualifying deposit only" via the unique `deposit_request_id`). Both sources respect new `AppSetting` keys: `referral_source_plan_purchase_enabled` (default true, preserves old behavior), `referral_source_deposit_enabled` (default false, opt-in), `referral_min/max_qualifying_amount` (blank = unbounded, same convention as `upi_min_amount`).
- **Admin management**: new `ReferralCommissionController` (index + approve/reject/adjust/reverse) and `Admin::referral-commissions` view - dashboard stat tiles (Total/Active Referrals, Total/Pending/Paid Commission), status-tab history table matching `deposits.blade.php`'s structure, one shared reason-modal (not three near-identical ones) for Reject/Adjust/Reverse driven by `data-*` attributes, same pattern as the existing Ban-user modal. Approve has no reason (plain `confirm()`, matches deposit-approve's asymmetry); Reject/Adjust/Reverse all require one, validated the same way `toggleBanUser()` does. New `manage_referrals` permission in `AdminRoles` (granted to super_admin/manager/finance). New sidebar nav entry.
- **Anti-fraud**: added an explicit self-referral guard at the actual `referred_by`-assignment point in `PhoneAuthController::attributeReferral()` and `GoogleAuthController::callback()` (comparing the referrer's phone/email to the new account's) - defense in depth on top of the existing link-capture check, though by construction a brand-new account's phone/email can't already collide with an existing user's (unique constraints), so this mostly future-proofs against a refactor accidentally breaking that guarantee rather than closing a currently-exploitable hole.
- **Rewards page**: `RewardsController` now groups (not `keyBy`s) commissions per referred user, since one referred user can now generate two independent commission rows (one per source) - `keyBy` would have silently dropped one. Split `totalCommission` into `pendingCommission`/`paidCommission`; updated the "credited instantly" hero copy (now false) and added a pending-amount badge.
- Added `tests/Feature/ReferralCommissionWorkflowTest.php` (9 tests) covering the full money-safety surface: pending-on-purchase, approve credits + ledger row, reject requires reason + no money movement, adjust while pending, reverse debits a paid row, deposit-source independence from plan-purchase, min-qualifying-amount gate, source-toggle gate, and the Rewards page split rendering. Full suite: **148 passed (528 assertions)**, Pint clean. Verified the two hand-written Blade views (`referral-commissions.blade.php`, `settings.blade.php` additions) via a real browser session per this repo's documented Tinker-vs-real-HTTP preference - stat tiles, status tabs, and the Adjust modal (prefilled amount, correct uuid-routed form action) all confirmed working end-to-end; Approve confirmed via `form.submit()` (bypassing the native `confirm()` dialog, which blocks the browser-automation tool's async round-trip - a tooling limitation, not an app issue).

## 2026-08-14 — Dead-code scan/cleanup + Admin "Ban user" button invisible due to a stale CSS build

User asked for a Python script to scan migrations/views/models for dead code, then iterated it into a relation-graph checker (`scripts/detect_dead_code.py`, not committed to app code but kept in the repo): every symbol (view key, model class, migration table, controller action) is defined at an exact file:line, then every other line in the repo is searched for a relation to it via several independent, structurally different checks (direct string search, a reverse-index parsed from every call site, filename search, import/instantiation parse, route-pair parse) - only flagged if *all* checks agree nothing references it. Flagged items also get automatic corroborating evidence (leading-comment keywords, git history). Report written to `docs/dead_code_report.md`.

- Deleted three confirmed-dead Blade views: `app/Modules/Auth/Views/auth-overlay.blade.php`, `app/Modules/Settings/Views/modals.blade.php`, `resources/views/components/plan-badge.blade.php`.
- While investigating `auth-overlay`, found the "Login / Sign Up" button in the "Login Required" popup (`resources/views/components/popups.blade.php`) was non-functional: its `onclick` referenced `window.openAuth` (defined nowhere in the codebase) and fell back to `getElementById('auth-overlay')`, which returned `null` since that view was never rendered anywhere - the whole handler was a no-op besides closing the popup. Replaced with `href="{{ route('login') }}"`, matching every other login CTA in the app (welcome, sidebar, home, profile, rewards, plan-details).
- Separately, user reported the admin "Ban user" confirm button in the ban-reason modal (`users.blade.php` / `user-profile.blade.php`) wasn't showing, only Cancel. Root cause **was not the markup** - both buttons were always present and correctly positioned. `bg-red-600` is used *only* in these two ban-modal partials, added after the last `npm run build`; Tailwind v4 (`@tailwindcss/vite`) only emits CSS for classes it sees at build time, so the compiled bundle was missing that utility - the button rendered as white text on a transparent background sitting on the white modal card (invisible, not missing). Fixed by rebuilding (`npm run build`); verified via a real Playwright-less browser session (`chrome_javascript` computed-style check) that the button now renders solid red.
- **Gotcha worth remembering**: any new Tailwind utility class added to a Blade file needs `npm run build` (or the Vite dev server running) before it actually renders - otherwise it silently has zero effect instead of erroring, and the symptom looks like a missing/broken element rather than a build problem.
- Full suite not re-run for this change (view deletions + one CSS rebuild + one onclick→href swap, no controller/model changes); real-browser-verified per project convention instead. Pint not applicable (no PHP logic changed, only a Blade attribute swap).
